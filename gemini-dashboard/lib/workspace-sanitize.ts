  ...
  if (worker.policy) {
    sanitizedPolicy = {
      ...
      readScope: worker.policy.readScope
        ? { ...worker.policy.readScope, deny: worker.policy.readScope.deny?.map(f => sanitizePath(f, repoRoot)) }
        : undefined,
      writeScope: worker.policy.writeScope
        ? {
            ...worker.policy.writeScope,
            expected: worker.policy.writeScope.expected?.map(f => sanitizePath(f, repoRoot)),
            derived_approved: worker.policy.writeScope.derived_approved?.map(f => sanitizePath(f, repoRoot)),
            sensitive: worker.policy.writeScope.sensitive?.map(f => sanitizePath(f, repoRoot)),
            forbidden: worker.policy.writeScope.forbidden?.map(f => sanitizePath(f, repoRoot)),
          }
        : undefined,
      mergeScope: worker.policy.mergeScope
        ? {
            ...worker.policy.mergeScope,
            expected: worker.policy.mergeScope.expected?.map(f => sanitizePath(f, repoRoot)),
            deny: worker.policy.mergeScope.deny?.map(f => sanitizePath(f, repoRoot)),
          }
        : undefined,
      entries: worker.policy.entries?.map(entry => ({
        ...
        matchedPattern: entry.matchedPattern
          ? sanitizePath(entry.matchedPattern, repoRoot)
