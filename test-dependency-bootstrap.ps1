...
function New-MockNpmEnv([string]$MockDir, [string]$Behavior = 'success') {
  ...
  $logPath = (Join-Path $MockDir 'npm-invocations.log').Replace('\', '/')
  $pidPath = (Join-Path $MockDir 'child-pids.log').Replace('\', '/')
...
try {
  ...
  $mockInvocations1 = if (Test-Path -LiteralPath $mock1.LogPath) { @(Get-Content -LiteralPath $mock1.LogPath | Where-Object { $_ }) } else { @() }
