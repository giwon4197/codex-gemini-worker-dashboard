...
export function buildProjectWorkGraph(
  ...
  // A run that died before the router produced any task or worker (launcher
  // error, dirty tree, missing tools) has no plan and no branch on record.
  // Only the request node is real; never synthesize a completed plan or a
  // pending placeholder worker for it.
  const diedBeforePlanning =
    normalizedRunStatus === 'failed' && tasks.length === 0 && workers.length === 0;
  ...
  const effectiveTasks = diedBeforePlanning
    ? []
    : tasks.length > 0
      ? tasks
  ...
  if (mergeNode) {
    ...
    const tip = diedBeforePlanning ? reqNode : planNode;
    tip.isTip = true;
    tips.push(tip);
  }
