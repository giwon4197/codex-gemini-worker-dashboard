...
import { evaluateRunLiveness, getEffectiveLivenessOptions, isProcessAlive, STALE_PROCESS_MISMATCH_REASON } from './process-liveness.ts';
...
export function resolveRequiredTools(options?: ToolResolutionOptions): ToolResolutionResult {
  ...
  const findCodex = (): string | null => {
    ...
    // Same policy as codex-conversation.ts: codex-router.ps1 calls codex.exe by
    // name, so a .cmd shim directory on PATH would not satisfy it.
    const pathFound = searchInPath(isWin ? ['codex.exe'] : ['codex']);
    if (pathFound) return pathFound;

    if (isWin) {
      const npmVendorExe = path.join(
        env.APPDATA || '', 'npm', 'node_modules', '@openai', 'codex', 'node_modules',
        '@openai', 'codex-win32-x64', 'vendor', 'x86_64-pc-windows-msvc', 'bin', 'codex.exe'
      );
      if (env.APPDATA && testFile(npmVendorExe)) return npmVendorExe;
...
export function extractSanitizedFailureReason(
  ...
  if (rawCandidate) {
    ...
    if (cleanedLines.length > 0) {
      ...
      // Prefer explicit error lines; otherwise the log tail, never the Codex banner at the head.
      const errorLines = unique.filter(l => /error|exception|throw|오류|실패|찾을 수 없|않았습니다/i.test(l));
      const summary = (errorLines.length > 0 ? errorLines.slice(0, 2) : unique.slice(-2)).join('; ');
...
export async function settleRunState(
  ...
  // The router runs a Codex planning subcall before any manifest exists, which
  // routinely exceeds the timeout. A live process is never timed out here;
  // the timeout only applies when the PID is unknown or gone.
  if (!hasManifestEvidence && (isProcessTerminated || (isTimedOut && !isAlive))) {
    ...
    // Do not invent an exit code: the bootstrap writes one on exit; absent means it never got there.
...
export async function spawnRouterRun(options: {
  ...
  const childEnv: NodeJS.ProcessEnv = sanitizeSpawnEnv({
    ...process.env,
    ...options.env,
...
export async function retryRun(options: {
  ...
  // 3b. Never spawn a second planner while the original router process is still alive
  const originalPid = originalCompact.orchestratorProcessId;
  if (typeof originalPid === 'number' && originalPid > 0) {
    const liveness = getEffectiveLivenessOptions();
    const alive = liveness.processInfoResolver
      ? Boolean((await liveness.processInfoResolver(originalPid, liveness.platform)).alive)
      : liveness.isAlive
      ? liveness.isAlive(originalPid)
      : isProcessAlive(originalPid);
    if (alive) {
      const err = new Error('원본 실행 프로세스가 아직 종료되지 않았습니다. 종료된 뒤 다시 시도하세요.');
      (err as unknown as { code: string }).code = 'RETRY_NOT_PERMITTED';
      throw err;
    }
  }
