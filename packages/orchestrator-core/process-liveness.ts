 ...
 */
// ponytail: short TTL cache; the Windows path spawns tasklist + powershell synchronously
// and the extension polls every 2.5s, so uncached lookups stall the extension host.
const PROCESS_INFO_TTL_MS = 5_000;
const processInfoCache = new Map<number, { at: number; info: ProcessInfo }>();
...
export function getProcessInfo(
  ...
  if (!isProcessAlive(pid)) {
    processInfoCache.delete(pid);
    return { pid, alive: false, metadataAvailable: true };
  }
  const cached = processInfoCache.get(pid);
  if (cached && Date.now() - cached.at < PROCESS_INFO_TTL_MS) {
    return cached.info;
  }
  const info = resolveProcessInfo(pid, platform);
  processInfoCache.set(pid, { at: Date.now(), info });
  return info;
}

function resolveProcessInfo(pid: number, platform: NodeJS.Platform): ProcessInfo {
